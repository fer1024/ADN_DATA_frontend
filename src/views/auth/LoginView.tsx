...
export default function LoginView() {
  ...
  const { mutate } = useMutation({
    ...
    onSuccess: () => {
      // Limpia todo el cache para que no queden datos del usuario anterior
      queryClient.clear()
      navigate('/')
    }
  })
        ...
        <div className="bg-[#1e293b] shadow-2xl rounded-2xl border border-slate-700/50 overflow-hidden">
          <div className="p-4 sm:p-8">
            ...
            <form onSubmit={handleSubmit(handleLogin)} className="space-y-5" noValidate>
              ...
              <button
                type="submit"
                className="w-full py-3 px-4 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-lg shadow-lg shadow-cyan-900/20 transition-all active:scale-[0.98] mt-4"
              >
                Ingresar
              </button>
            </form>
          ...
          <div className="bg-[#2d3a4f]/30 p-4 border-t border-slate-700/50">
            <nav className="flex flex-col space-y-2 text-sm text-center">
              <Link to={'/auth/register'} className="text-slate-400 hover:text-cyan-400 transition-colors">
                ¿No tienes cuenta? <span className="font-semibold text-cyan-500">Regístrate</span>
              </Link>
              <Link to={'/auth/forgot-password'} className="text-slate-500 hover:text-slate-300 transition-colors text-xs">
